...
(function(a){
	a.fn.upload=function(D){
		...
		g='<svg xmlns="http://www.w3.org/2000/svg" class="delete" id="delete_img" onclick="delece_img({time()})" version="1" viewBox="0 0 1024 1024"><path fill="#fff" d="M512 70a439 439 0 0 1 442 442 439 439 0 0 1-442 442A439 439 0 0 1 70 512 439 439 0 0 1 512 70m0-40a482 482 0 1 0 0 964 482 482 0 0 0 0-964zm114 253v-1c0-21-17-38-38-38H436c-21 0-38 17-38 38v1H282v74h460v-74H626zM321 396v346c0 21 17 38 38 38h306c21 0 38-17 38-38V396H321zm114 306h-76V474h76v228zm115 0h-76V474h76v228zm115 0h-76V474h76v228z"/></svg>',
		E='<svg xmlns="http://www.w3.org/2000/svg" class="look" version="1" viewBox="0 0 1024 1024"><path fill="#fff" d="M451 835a386 386 0 1 1 0-771 386 386 0 0 1 0 771zm0-675a291 291 0 1 0 0 581 291 291 0 0 0 0-581zm450 798c-15 0-30-5-42-17L658 740a58 58 0 0 1 83-82l201 201a58 58 0 0 1-41 99"/></svg>',d=a('<input type="file" name="file" />'),b=a('<input type="hidden" />'),
		j,u,t,o,n,m,y,out,
		B={
			...
			size:r.attr("data-size")?r.attr("data-size"):20480,value:r.attr("data-value")?r.attr("data-value"):""};
			r.append(
				A.attr("data-num",B.num).attr("data-type",B.type)).append(d.attr("multiple",B.num>1?"multiple":false)).append(b.attr("name",B.inputname)).addClass(B.num>1?"multiple":"one").on("click","li.add",function(){d.click()}).on("change","input[type='file']",function(H){var G=H.target.files;var i=[];for(var F in G){if(typeof G[F]=="object"){i.push(G[F])}}C(i)}).on("click","li.error",function(i){z(a(i.currentTarget))}).on("click","svg.delete",function(i){z(a(i.currentTarget).parent("li.success"))}).on("click","svg.look",function(i){if(D&&typeof D=="function"){D(r,a(i.currentTarget).parent("li.success").data("url"))}}).on("mousedown","li.success",function(I){var i=a(I.target);j=a(this);var J=j.offset();var H=r.offset();var G=J.left;var F=J.top;u=parseInt(G)+(I.pageX-parseInt(G))+(H.left-J.left);t=parseInt(F)+(I.pageY-parseInt(F))+(H.top-J.top);m=j.width()+2+j.css("margin-right").replace("px","")*1;y=j.height()+2+j.css("margin-bottom").replace("px","")*1;o=false;n=false});
			if(B.value){
				var p=B.value.split(","),s=0,h="";
				var f=0;
				for(var x in p){
					if(s<B.num&&p[s]){
						h=a("<li class='item success'></li>").append(g).append(typeof D=="function"?E:"").data("url",p[s]).attr("data-filename",c(p[s])).css("background-image","url('"+p[s]+"')").insertBefore(A);r.data("num",++f).removeClass("empty");q()
					}
					s++
				}
			}else{
				r.data("num",0).addClass("empty")}a(document).on("mousemove",function(G){
					if(j){
						var F=G.pageX-u;var J=G.pageY-t;var I=j.index();var H=r.offset();var i=r.height()?r.height():134;j.css({top:J,left:F,zIndex:999,position:"absolute"});o=G.pageX-H.left;n=G.pageY-H.top;
						if(n>(i+20)||n<0){
							j.addClass("delete")
						}else{
						...
						r.children("li").eq(o).not(j).css("margin-left",m)}}).on("mouseup",function(){if(j){if(j.hasClass("delete")&&j.data("filename")){z(j);
							j=false;r.children("li").not(".add").css({"position":"relative","top":0,"left":0,"margin-left":0,"zIndex":0});
...
a.ajax({
	...
	success:function(M){
		...
		if(M.code){
			J.addClass("success").append(g).append(typeof D=="function"?E:"").data("url",M.msg).attr('index',M.msg)
		}else{
		...
		xhr:function(){
			...
			M.upload.addEventListener("progress",function(N){
				if(N.lengthComputable){
					...
					J.children(".progressnum").text(O)}},false);return M},
			error:function(){
...
function l(G,L){
	...
	if(K){
		i.append(k(J)).addClass("error").append("<div class='filename'>"+G.name+"</div>").attr("data-error",K).insertBefore(A);r.removeClass("empty");q();
		return false
	}
...
function q(){
	...
	if(r.children("li").not(".add").size()>=B.num&&B.num>1){
		...
		function z(i){
			$.ajax({
				   type: "get",
				  url: "/admin/goods/goods_img_delete",  // 测试
				   // url: "goods_img_delete",
